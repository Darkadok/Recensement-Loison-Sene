...
/****** On remplit la dernière case ******/
		...
		((*tab_region)+ **taile_tab_region)->nom_reg = malloc(sizeof(wchar_t)* (wcslen(nom_reg) + 1));

		wcscpy(((*tab_region)+ **taile_tab_region)->nom_reg, nom_reg);

		((*tab_region)+ **taile_tab_region)->tab_departement = NULL;
		((*tab_region)+ **taile_tab_region)->taille_tab_departement = NULL; 
